...
import {updateEtudiant } from "../Admin.service";

export function UpdateForm(props) {
    const {open , handleOpen , setUpdate , groupes , etudiant} = props
    ...
    useEffect(() => {
            const { nom, prenom, date_naissance, Adresse, id_groupe, Telephone, email, Telephone_responsable, email_responsable} = etudiant;
            setNom(nom);
            setPrenom(prenom);
            ...
            setTele(Telephone);
            setEmail(email);
            setTeleR(Telephone_responsable);
            setEmailR(email_responsable);
    }, [etudiant]);
        ...
        if (isValid) {
            try {
                ...
                if (confirmer) {
                    ...
                    await updateEtudiant(etudiant1 , etudiant.id);
                    setUpdate(etudiant1)
                    swal("Bravo", "étudiant modifié avec succès.", { icon: "success" });
                    setNom('');
                    setPrenom('');
                    setDate('');
                    setAdresse('');
                ...
                swal("Erreur", "Une erreur s'est produite lors de la modification d'étudiant.", { icon: "error" });
            }
        }
    }
                        ...
                        <TabsBody className="!overflow-x-hidden !overflow-y-visible">
                            ...
                            <div className="mt-12 flex flex-col gap-7">
                                ...
                                <div className="flex flex-col w-1/2">
                                    ...
                                    <div className="flex flex-col w-1/2">
                                        <Select label="Groupe" color="black" className="Select Version" onChange={handleSelectChange} value={id_groupe}>
                                            {groupes.map(gp => (
