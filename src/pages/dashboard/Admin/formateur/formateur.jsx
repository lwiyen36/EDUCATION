...
import {getFormateurs , PrepareFormateur} from "../Admin.service";
...
//    }
  ...
  const fetchData = async () => {
    try {
      ...
      setData(response.data.formateurs);
    } catch (error) {
  ...
  const prepare = async () => {
    try {
        const response = await PrepareFormateur();
  ...
  return (
    <div className="mt-12 mb-8 flex flex-col gap-12">
      <Card>
        ...
        <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
          <table className="w-full min-w-[800px] table-auto">
            <thead>
              <tr>
                {["id", "Nom ", "Prenom", "email", "telephone", "matiere" , "heure de travail" , "Options"].map((el) => (
            ...
            <tbody>
              {data.map((fr, key) => (
                ...
                <Typography as="a" href="#" className="text-xs font-semibold text-blue-gray-600 text-center">
                  ...
                  <td className={`py-3 px-5 ${key === data.length - 1 ? "" : "border-b border-blue-gray-50"}`}>
                    <div className="flex items-center justify-center gap-3">
                      <Button ripple={true} variant="text" onClick={() => { editEtudiant(fr.id) ; }}>
                      ...
                      <Button ripple={true} variant="text" onClick={() => removeEtudiant(fr.id)}>
      ...
      <AddForm open ={open} handleOpen={handleOpen} matieres={matieres} setAdd={setAdd} />
    </div>
  );
}
